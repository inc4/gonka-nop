...
func Header(text string) {
	...
	_, _ = dimmed.Println(repeat("─", len(text)))
}

// PhaseStart prints phase start indicator
func PhaseStart(number int, name string) {
	fmt.Println()
	_, _ = cyan.Printf("━━━ Phase %d: %s ", number, name)
	_, _ = dimmed.Println(repeat("━", 40-len(name)))
}

// PhaseComplete prints phase completion
func PhaseComplete(name string) {
	_, _ = green.Printf("✓ %s complete\n", name)
}
