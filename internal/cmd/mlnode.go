...
var mlNodeAddConfigFile string
...
var mlNodeAddCmd = &cobra.Command{
	Use:   "add",
	Short: "Register a new ML node with the network node",
	Long: `Register a new ML node with the network node's Admin API.

Two modes:
  From file:    gonka-nop ml-node add --config mlnode-registration.json
  Interactive:  gonka-nop ml-node add (prompts for host, ports, model)

The registration file is generated by 'gonka-nop setup --type mlnode' on the GPU server.`,
	RunE: runMLNodeAdd,
}
...
func runMLNodeAdd(_ *cobra.Command, _ []string) error {
	var payload []byte

	if mlNodeAddConfigFile != "" {
		// Mode 1: From file
		data, err := os.ReadFile(mlNodeAddConfigFile) // #nosec G304 - user-provided path
		if err != nil {
		...
		if err := json.Unmarshal(data, &check); err != nil {
			return fmt.Errorf("invalid JSON in %s: %w", mlNodeAddConfigFile, err)
		}
		payload = data
		ui.Info("Loading ML node config from %s", mlNodeAddConfigFile)
	} else {
		// Mode 2: Interactive
		reg, err := promptMLNodeRegistration()
		if err != nil {
			return err
		}
	...
	url := adminURL + "/admin/v1/nodes"

	resp, err := client.Post(url, "application/json", bytes.NewReader(payload))
	if err != nil {
		return fmt.Errorf("connecting to Admin API at %s: %w", url, err)
	}
	...
	// Parse response to show the registered node ID
	var result struct {
		ID string `json:"id"`
	}
	if json.Unmarshal(body, &result) == nil && result.ID != "" {
		_, _ = color.New(color.FgGreen).Printf("ML node %q registered successfully.\n", result.ID)
	} else {
		_, _ = color.New(color.FgGreen).Println("ML node registered successfully.")
	}

	ui.Detail("Verify with: gonka-nop ml-node list")
	return nil
}

// promptMLNodeRegistration interactively builds an ML node registration payload.
func promptMLNodeRegistration() (*status.AdminNodesNodeInfo, error) {
	ui.Header("Register New ML Node")

	id, err := ui.Input("Node ID (unique identifier):", "node1")
	if err != nil {
		return nil, err
	}

	host, err := ui.Input("ML node host (IP or hostname, no http://):", "")
	if err != nil {
		return nil, err
	}
	if host == "" {
	...
	// Strip http:// prefix if user accidentally added it
	host = strings.TrimPrefix(host, "http://")
	host = strings.TrimPrefix(host, "https://")

	inferencePort, err := ui.Input("Inference port:", "5050")
	if err != nil {
		return nil, err
	}

	pocPort, err := ui.Input("PoC/management port:", "8080")
	if err != nil {
		return nil, err
	}

	maxConcurrent, err := ui.Input("Max concurrent requests:", "500")
	if err != nil {
		return nil, err
	}

	model, err := ui.Input("Model name:", "Qwen/Qwen3-235B-A22B-Instruct-2507-FP8")
	if err != nil {
		return nil, err
	}

	tpSize, err := ui.Input("Tensor parallel size:", "4")
	if err != nil {
		return nil, err
	}

	reg := &status.AdminNodesNodeInfo{
		ID:            id,
		Host:          host,
		InferencePort: parseInt(inferencePort, 5050),
		PoCPort:       parseInt(pocPort, 8080),
		MaxConcurrent: parseInt(maxConcurrent, 500),
		Models: map[string]status.AdminModelConfig{
			model: {Args: []string{"--tensor-parallel-size", tpSize}},
		},
	}
